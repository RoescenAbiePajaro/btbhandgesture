...
class KeyboardInput:
    ...
    def process_key_input(self, key):
        ...
        # === DEBOUNCE UNDO SAVING ===
        # Only save undo state on: first char, Enter, or every N keystrokes / time
        ...
        if key == 13:  # Enter
            if selected_index >= 0:
                ...
                if self.text.strip():
                    self.save_state()  # expensive – only when confirming real text
                    ...
                    self.text = ""
                    self.current_input_position = (640, 360)
            should_save_undo = False  # already saved above

        elif key == 8:  # Backspace
            if selected_index >= 0:
                old_text = self.text_objects[selected_index]['text']
                self.text_objects[selected_index]['text'] = old_text[:-1]
                if not self.text_objects[selected_index]['text']:
                    self.delete_selected()
                should_save_undo = len(old_text) == 1 or len(old_text) % 5 == 0
            else:
                if self.text:
                    old_len = len(self.text)
                    self.text = self.text[:-1]
                    should_save_undo = (old_len == 1) or (old_len % 6 == 0)

        elif 32 <= key <= 126:  # Printable ASCII
            char = chr(key)
            if selected_index >= 0:
                old_text = self.text_objects[selected_index]['text']
                self.text_objects[selected_index]['text'] += char
                should_save_undo = len(old_text) == 0 or len(old_text) % 5 == 0
            else:
                old_len = len(self.text)
                self.text += char
                should_save_undo = old_len == 0 or old_len % 5 == 0

            # Smooth animation – keep it, it's lightweight
            if len(self.smooth_text) >= 30:
                self.smooth_text.popleft()
            self.smooth_text.append({
                'char': char,
                'alpha': 0, 'scale': 0.8, 'y_offset': 10,
                'elapsed': 0, 'target_pos': len(self.text)-1 if selected_index < 0 else len(self.text_objects[selected_index]['text'])-1
            })

        else:
            return False

        # === ONLY SAVE UNDO STATE SPARINGLY ===
        if should_save_undo:
            # This is the expensive part – now happens ~5–10× less
            self.save_state()
